...
def process_gpu_usage(input_file_name):
    ...
    write_dict_to_file(user_dict, "/tmp/sacct_tmp_files/user_dictionary.csv")
    write_dict_to_file(group_dict, "/tmp/sacct_tmp_files/group_dictionary.csv")
    write_dict_to_file(partition_dict, "/tmp/sacct_tmp_files/partition_dictionary.csv")
...
def run_command(s_date, e_date):
    ...
    with open("/tmp/sacct_tmp_files/today_sacct.data", "w") as output_file:
...
def getdata_current_or_missing_dates():
    time_stamp_file = "/tmp/sacct_tmp_files/sacct_collect_timestamp.data"
    ...
    with open(time_stamp_file, 'a') as file:
        for start_date, end_date in missing_dates:
            ...
            process_gpu_usage("/tmp/sacct_tmp_files/today_sacct.data")
            file_pairs = [
